...
public class FirebaseHandler : MonoBehaviour {
	...
	//this class is intended to interact between the game-manager and the SimpleFirebaseUnity plugin

	private static string firebaseURL = "https://arg-spies.firebaseio.com/";

	void Start () {
		StartCoroutine(Tests());
	}

	//these are the callback handlers for SimpleUnityFirebase
	...
	IEnumerator Tests()
	{
		Debug.Log("------Waiting for 20s-----");
		yield return new WaitForSecondsRealtime(20.0f);
		Debug.Log("=====**Wait complete- attempting to access firebaseDB***======");
		...
		Firebase.Auth.Credential credential = Firebase.Auth.FacebookAuthProvider.GetCredential(Facebook.Unity.AccessToken.CurrentAccessToken.ToString());
		SimpleFirebaseUnity.Firebase firebase = SimpleFirebaseUnity.Firebase.CreateNew (FirebaseHandler.firebaseURL, credential.ToString());
		Debug.Log("credential: "+credential.ToString());
		Debug.Log("Token: ");
